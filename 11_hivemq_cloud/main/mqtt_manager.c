...
#define MQTT_CONNECT_TIMEOUT_MS 15000  /* TLS handshake = plus long */

/* ------------------------------------------------------------------ */
/*  Ring buffer offline (identique au 10)                              */
/* ------------------------------------------------------------------ */
...
static void buf_push(const char *topic, const char *payload, int qos)
{
    ...
    if (s_buf_count == MQTT_OFFLINE_BUFFER_SIZE) {
        ESP_LOGW(TAG, "Buffer offline plein — overwrite");
                                ...
                                void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ESP_LOGI(TAG, "CONNACK recu — connecte a HiveMQ Cloud");
        s_connected = true;
    ...
    case MQTT_EVENT_DISCONNECTED:
        ESP_LOGW(TAG, "Deconnecte — buffer offline actif");
        s_connected = false;
        break;

    case MQTT_EVENT_PUBLISHED:
        ESP_LOGD(TAG, "PUBACK recu msg_id=%d", event->msg_id);
        break;
    ...
    case MQTT_EVENT_ERROR:
        ...
        if (event->error_handle->error_type ==
            ...
            ESP_LOGE(TAG, "Connexion refusee — verifier credentials");
        }
...
esp_err_t mqtt_manager_init(void)
{
    ...
    esp_mqtt_client_config_t mqtt_cfg = {
        .broker = {
            ...
            /* Validation TLS via bundle Mozilla intégré ESP-IDF */
    ...
    ESP_LOGE(TAG, "Timeout/echec connexion HiveMQ Cloud");
    return ESP_FAIL;
}
