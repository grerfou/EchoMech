...
void app_main(void)
{
    ...
    /* MQTT avec callback OTA */
    ret = mqtt_manager_init_with_ota_cb(on_ota_trigger);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "MQTT KO");
        goto sleep;
    }
    ...
    if (s_ota_requested) {
        ...
        snprintf(status_topic, sizeof(status_topic),
                 "device/%s/ota/status", device_id);
        mqtt_manager_publish(status_topic,
                             "{\"status\":\"downloading\"}", 0);
        vTaskDelay(pdMS_TO_TICKS(500));

        ota_result_t result = ota_manager_start(
            s_ota_url,
            s_ota_version[0] ? s_ota_version : NULL,
            s_ota_sha256[0]  ? s_ota_sha256  : NULL
        );

        if (result == OTA_RESULT_SAME_VERSION) {
            mqtt_manager_publish(status_topic,
                                 "{\"status\":\"same_version\"}", 0);
